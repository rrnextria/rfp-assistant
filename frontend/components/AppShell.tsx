...
import { useBrand } from "@/components/branding/BrandThemeProvider";

// Primary nav order: RFPs first, /ask demoted below admin.
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const appTitle = tenant?.display_name?.trim() || "RFP Assistant";
  const logoUrl = tenant?.brand?.logo_url;
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          <Link href="/rfps" className="flex items-center gap-2">
            {logoUrl && (
              ...
              <img src={logoUrl} alt={appTitle} className="h-6 w-auto" />
            )}
            <span
              className="text-xl font-bold"
              style={{ color: "var(--brand-primary, #1d4ed8)" }}
            >
              {appTitle}
            </span>
            {!tenant && (
