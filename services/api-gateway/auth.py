...
# --- Teams -------------------------------------------------------------------
# Restored from prior working tree: the users/companies admin UIs assume
# teams are tenant-scoped and addressable as /teams.
...
async def list_teams(
    ...
    rows = await db.execute(
        text("SELECT id::text, name FROM teams WHERE tenant_id = :t ORDER BY name"),
...
async def create_team(
    ...
    if current_user["role"] != "system_admin":
        raise HTTPException(403, "Forbidden")
    existing = await db.execute(
        text("SELECT id FROM teams WHERE tenant_id = :t AND name = :n"),
        {"t": current_user["tenant_id"], "n": req.name},
    )
    if existing.first():
        raise HTTPException(409, "Team already exists")
    team_id = str(uuid.uuid4())
    await db.execute(
        text("INSERT INTO teams (id, tenant_id, name) VALUES (:i, :t, :n)"),
        {"i": team_id, "t": current_user["tenant_id"], "n": req.name},
    )
    await db.commit()
...
async def delete_team(
    ...
    if current_user["role"] != "system_admin":
        raise HTTPException(403, "Forbidden")
    r = await db.execute(
        text("DELETE FROM user_teams WHERE team_id = :i"), {"i": team_id})
    await db.execute(
        text("DELETE FROM teams WHERE id = :i AND tenant_id = :t"),
        {"i": team_id, "t": current_user["tenant_id"]},
    )
    await db.commit()


# --- Customers (legacy alias for /companies) ---------------------------------
# The frontend's existing admin/customers page hits /api/customers — alias here
# rather than rename the underlying companies table.
...
async def list_customers(db: AsyncSession = Depends(get_db)):
    rows = await db.execute(
        text("SELECT id::text, name, industry, region FROM companies ORDER BY name"))
    return [{"id": r["id"], "name": r["name"],
             "industry": r["industry"] or "", "region": r["region"] or ""}
            for r in rows.mappings().all()]


# --- Settings (public app_name + logo) ---------------------------------------

settings_router = APIRouter(prefix="/settings", tags=["settings"])
general_router = APIRouter(prefix="/admin/general-settings", tags=["admin"])
llm_router = APIRouter(prefix="/admin/llm-settings", tags=["admin"])


async def _general_settings_dict(db: AsyncSession) -> dict:
    try:
        rows = await db.execute(
            text("SELECT key, value FROM general_settings "
                 "WHERE key IN ('logo_data_url', 'app_name')"))
        return {r["key"]: r["value"] for r in rows.mappings().all()}
    except Exception:
        # general_settings table may not exist in this deployment; return empty
        return {}


@settings_router.get("/general")
async def public_settings(db: AsyncSession = Depends(get_db)):
    data = await _general_settings_dict(db)
    return {"logo_data_url": data.get("logo_data_url", ""),
...
async def get_llm_settings(
    ...
    try:
        row = await db.execute(
            text("SELECT provider, model, api_key, azure_endpoint, azure_deployment "
    ...
    if not r:
        return {"provider": "openai", "model": "gpt-4o",
                "api_key_set": False, "azure_endpoint": "",
                "azure_deployment": "gpt-4o", "providers": {}}
    return {"provider": r["provider"], "model": r["model"],
            "api_key_set": bool(r["api_key"]),
            "azure_endpoint": r["azure_endpoint"] or "",
            "azure_deployment": r["azure_deployment"] or "gpt-4o",
            "providers": {}}
